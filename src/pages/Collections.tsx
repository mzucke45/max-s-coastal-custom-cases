          ...
          <StaggerContainer className="max-w-4xl mx-auto space-y-6">
            {isLoading ? (
              Array.from({ length: 3 }).map((_, i) => (
                <Skeleton key={i} className="aspect-[21/9] rounded-2xl" />
              ))
              ...
              collections.map((collection) => (
                ...
                >
                  <Link to={`/collections/${collection.id}`} className="group block">
                    <div className="relative overflow-hidden rounded-2xl aspect-[21/9] bg-muted shadow-md hover:shadow-xl transition-shadow duration-500">
                      ...
                      <div className="absolute inset-0 bg-gradient-to-r from-ocean-deep/70 via-ocean-deep/30 to-transparent flex items-center p-8 md:p-12">
                        <div className="max-w-sm">
                          <h2 className="font-display text-2xl md:text-3xl font-bold text-primary-foreground mb-2">{collection.name}</h2>
