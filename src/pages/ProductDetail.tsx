        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16">
            <motion.div
              ...
              className="overflow-hidden rounded-2xl aspect-[3/4] bg-muted shadow-lg"
            >
              {product.image_url ? (
            ...
            >
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-coral mb-3 font-body">{product.category}</p>
              ...
              <div className="space-y-4">
                  ...
                  <Select value={selectedModel} onValueChange={setSelectedModel}>
                    ...
                    <SelectContent>
                      {Object.entries(brandGroups).map(([brand, models]) => (
                        <div key={brand}>
                          <div className="px-2 py-1.5 text-xs font-bold text-muted-foreground uppercase tracking-wider">{brand}</div>
                          {models.map((model) => (
                ...
                {selectedModel && (
                  <Link to={`/designer?product=${product.id}&model=${selectedModel}`} className="block">
                    <Button
                      variant="outline"
                      className="w-full rounded-full h-12 text-sm font-semibold tracking-wide btn-squish"
                    >
